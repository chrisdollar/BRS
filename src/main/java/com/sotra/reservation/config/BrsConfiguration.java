...
public class BrsConfiguration {
    ...
    public Docket swaggerBRSApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("BRS")
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.sotra.reservation.controller.v1.api"))
    ...
    public Docket swaggerUserApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("User")
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.sotra.reservation.config"))
                .paths(PathSelectors.any())
                .build()
                .apiInfo(apiInfo())
                .securitySchemes(Arrays.asList(apiKey()));
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Bus Reservation System - REST APIs")
                .description("Sotra Reservation de Bus Système.").termsOfServiceUrl("")
                .contact(new Contact("Administator", "https://contact.sotra-brs.ci", "admin@sotra-brs.ci"))
